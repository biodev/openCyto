...
Note that for `2d` gate,  `marker/channel` names in `pop` column are optional (sometime impossible when they contain the special characters such as `+/-`) since they already defined in `dim` column and population patterns are soly determined by `+` and `-` symbols.
...
For example, we can set `pop` to `CD4+/-CD8+/-` to keep all of four quadrants.
